...
def create_product():
  ...
  if product_body is not None and "price" in product_body:
    product = set_product_price(product_id, product_body["price"], request_id).get()
  if product_body is not None and "stock" in product_body:
    product = add_product_stock(product_id, product_body["stock"], request_id).get()
